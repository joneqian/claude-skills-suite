...
| left-icon | 输入框左侧图标名称或图片链接，等同于 Icon 组件的name 属性 | string | `search` |
| right-icon | 输入框右侧图标名称或图片链接，等同于 Icon 组件的name 属性 | string | - |
...
| click-right-icon`3.4.0` | 点击右侧图标时触发 | event: MouseEvent |
